...
def makeInstrumentViewPlot(filename, outputdir):
    ...
    for i in range(int(nbanks)):
        ax.pcolormesh(th[i*256*256:(i+1)*256*256].reshape(256,256),
                      ...
                      #norm=LogNorm(vmin=0.1,vmax=0.80*intensity.max()))
...
def publish_plots(run_number):
    ...
    #plot_html_inst += '<div><img style="max-width:90%" src="/static/web_monitor/images/{0}_fig1.png" alt="Overview"></div>\n'.format(run_number)
...
def createMTZFile(d, out_dir, run_number):
    ...
    #outputFilenameTemplate = out_dir+'%s_ws_%i_mandi_reduced_jan19veldetcal.%s' #String with output file format.  %s will be replaced by file
    ...
    for rn in range(first_run_number, run_number+1):
        ...
        if (os.path.isfile(paramsFileName) and os.path.isfile(peaksFileName) and 
            ...
            #dfTPF = pd.DataFrame(mtd['peaks_ws_profile'].toDict())
    ...
    rIDX = gIDX & (df['RunNumber'] == run_number)
    ...
    if force_lattice_parameters:
        ...
        FindUBUsingLatticeParameters(PeaksWorkspace=pwsPF, a=a, b=b, c=c, alpha=alpha, beta=beta, gamma=gamma,
...
def do_reduction(filename,outputdir):
    ...
    """
    #Set up profile fitting
    ipts_number = re.search('IPTS-(\d+)', filename).group(1)
    python_command = '/SNS/users/ntv/workspace/mantid/release/bin/mantidpython'
    script_name = '/SNS/snfs1/instruments/MANDI/shared/autoreduce/mandi_singlerun.py'
    config_filename = '/SNS/snfs1/instruments/MANDI/IPTS-{0}/shared/autoreduce/mandi_autoreduce.config'.format(ipts_number)
    nxs_filename = filename 
    output_dir = outputdir 
    run_number = run_number
    load_venv_command = 'source /SNS/snfs1/instruments/MANDI/shared/autoreduce/venv/bin/activate; ' 

    mtz_script = '/SNS/snfs1/instruments/MANDI/shared/autoreduce/mandi_createmtz.py'
    if len(config_file_list) != 1:
        raise UserWarning("Config file {0} does not exist.  Cannot do profile fitting.".format(config_filename))
    else: #We have a config file we can use
        command_pf = load_venv_command + python_command + ' ' + script_name + ' ' + config_filename + ' ' + nxs_filename + ' ' + output_dir + ' ' + str(run_number)
        print command_pf
        subprocess.Popen(command_pf, shell=True) #Does the profile fitting
        command_mtz = load_venv_command + python_command + ' ' + mtz_script + ' ' + config_filename + ' ' + output_dir + ' ' + str(run_number)
        print command_mtz
        subprocess.Popen(command_mtz, shell=True)
    """
