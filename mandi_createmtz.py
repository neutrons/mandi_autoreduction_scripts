...
def createMTZFile(d, out_dir, run_number):
    ...
    for rn in range(first_run_number, run_number+1):
        ...
        if (os.path.isfile(paramsFileName) and os.path.isfile(peaksFileName) and 
            ...
            #dfT.drop(columns=['BankName', 'BinCount', 'DetID', 'TOF', 'Wavelength','QLab', 'QSample',
            #                  'dQ', 'newQ', 'scale3d' ] )
            dfList.append(dfT)

            if len(runNumbersProcessed) == 1: #First peak we've added
                pwsSPH = CloneWorkspace(InputWorkspace=peaks_ws, OutputWorkspace='pwsSPH')
                pwsPF = CloneWorkspace(InputWorkspace=peaks_ws_profile, OutputWorkspace='pwsPF')
            else: #Append the current workspaces
                pwsSPH = CombinePeaksWorkspaces(LHSWorkspace=pwsSPH, RHSWorkspace=peaks_ws, OutputWorkspace=pwsSPH)
                pwsPF = CombinePeaksWorkspaces(LHSWorkspace=pwsPF, RHSWorkspace=peaks_ws_profile, OutputWorkspace=pwsPF)
